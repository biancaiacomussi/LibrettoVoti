...
public class Libretto {
	 ...
	 * @return {@code true} nel caso normale, {@code false} se il voto esisteva gi�
	 * oppure era in conflitto
	 */
	public boolean add(Voto v) { //meglio questo metodo
		
		if(!this.esisteGiaVoto(v) && !this.votoConflitto(v)) {
		voti.add(v); //add ritorna un boolean true se la collection � cambiata
	 ...
	 * @return il {@link Voto}
	 */
	public Voto cercaEsame(String nomeEsame) { //si pu� fare con contains e indexOf
		/*for(Voto v : this.voti) {
		...
		if(pos==-1) //l'oggetto non � stato trovato
			return null;
		 else
	...
	/**
	 * Mi dice se il {@link Voto} {@code v} � in conflitto con uno dei voti
	 * esistenti. Se il voto non esiste, non c'� conflitto. Se esiste ed ha un punteggio diverso,
	 * c'� conflitto.
	 * @param v
